...
async def create_time() -> InlineKeyboardMarkup:
    ...
    inline_time.append([InlineKeyboardButton(text="Я передумал", callback_data="main")])
...
async def create_scheduler_all() -> None:
    ...
    """
    try:
        result: Sequence = await get_all_users()
        for user in result:
            await add_send_message(user.user_chat_id, user.time)
    except TelegramForbiddenError:
        pass
