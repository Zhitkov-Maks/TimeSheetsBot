...
async def get_prediction_month(callback: CallbackQuery, state: FSMContext):
    ...
    await asyncio.create_task(delete_message_after_delay(callback.message, 60))
    await asyncio.create_task(delete_message_after_delay(send_message, 60))


@predict.callback_query(F.data == "two_in_two")
async def two_in_to_get_prediction_scheduler(callback: CallbackQuery, state: FSMContext):
...
async def two_in_to_get_prediction_final(
    ...
    prediction_sum = await two_in_two_get_prediction_sum(message.from_user.id, data)
    send_message: Message = await message.answer(
...
async def five_days_get_prediction_scheduler(callback: CallbackQuery, state: FSMContext):
